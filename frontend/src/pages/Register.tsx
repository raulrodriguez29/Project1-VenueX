...
import { registerUser, type RegisterData } from '../api/auth.api';
...
const Register = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Pass the new user data to context
            login(data);
            navigate('/'); 
        } catch (err: any) {
    ...
    return (
        <div className="min-h-screen flex items-center justify-center hero-gradient font-body p-4">
            <div className="card p-8 rounded-2xl w-full max-w-md shadow-2xl border border-[#333]">
                <h2 className="text-4xl font-display text-brand mb-2 text-center">CREATE ACCOUNT</h2>
                <p className="text-gray-400 text-center mb-8 text-sm">Join VenueX to start booking exclusive events</p>
                ...
                <div className="mt-8 pt-6 border-t border-[#333] text-center">
                    <p className="text-gray-400 text-sm">
                        Already have an account? <Link to="/login" className="text-brand hover:underline font-semibold">Log in</Link>
                    </p>
                </div>
            </div>
        </div>
